...
def main():
    ...
    if predict_button:
        if starting_airport == destination_airport:
            ...
            st.error(
                f"❌ There is no flight from {starting_airport} to {destination_airport} airport. Please select different routes.")
            ...
            formatted_date = departure_date.strftime("%A, %B %d, %Y") if isinstance(departure_date,
                                                                                    datetime.date) else str(
                departure_date)
            formatted_time = departure_time.strftime("%I:%M %p") if isinstance(departure_time, datetime.time) else str(
                departure_time)
